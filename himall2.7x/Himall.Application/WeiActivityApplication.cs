...
namespace Himall.Application
{
    public class WeiActivityApplication
    {
        ...
        public static ObsoletePageModel<WeiActivityModel> Get(string text, WeiActivityType type, int pageIndex, int pageSize, bool? isIntegralActivity = null, bool isShowAll = true)
        {
            ObsoletePageModel<WeiActivityInfo> weiInfo = _iActivityInfoService.Get(text, type, pageIndex, pageSize,  isIntegralActivity, isShowAll);
            var datas = weiInfo.Models.ToList().Select(m => new WeiActivityModel()
            {
                ...
                codeUrl = m.CodeUrl,
                addDate = m.AddDate,
                totalNumber = WeiActivityWinApplication.GetWinNumber(m.Id, "ALL"),
                winNumber = WeiActivityWinApplication.GetWinNumber(m.Id, "True")
            }).ToList();

            ObsoletePageModel<WeiActivityModel> t = new ObsoletePageModel<WeiActivityModel>();
                ...
                if (i % 2 == 0)
                {
                    ...
                    else //大于4组奖项  未中奖显示不能均匀分布 
                    {
                        ...
                        else
                        {
                            ...
                            {
                                awardModel.couponName = CouponApplication.GetCouponInfo(long.Parse(item[awardNum].CouponId.ToString())).CouponName;
                            }
        ...
        public static  WeiActivityInfo GetActivityInfo(long id)
        {
          return   _iActivityInfoService.GetActivityModel(id);
        }
