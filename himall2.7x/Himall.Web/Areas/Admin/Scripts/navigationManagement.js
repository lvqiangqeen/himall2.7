...
function Delete(id) {
    $.dialog.confirm('确定删除该条记录吗？', function () {
        ...
        $.post("./Delete", { id: id }, function (data) {
            if (data.success) { $.dialog.tips('删除成功'); $("tr[swapid='" + id + "']").remove(); ReSetSwapIcon(); }
            else { $.dialog.errorTips('删除失败！' + data.msg); }
            loading.close();
        });
    });
}
        ...
        $.dialog({
            ...
            ok: function () {
                ...
                if (navname == "" || navurl == "") {
                    ...
                    var loading = showLoading();
                    $.post("./Add", { Name: navname, Url: navurl,EnableDelete:1 },
                        function (data) {
                            loading.close();
                            if (data.success) {
