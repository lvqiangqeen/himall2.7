        ...
        public static void AddBrowsingProduct(long productId, long userId = 0)
        {
            ...
            else
            {
                foreach (var item in productIdList)
                {
                    try
                    {
                        ServiceHelper.Create<IProductService>().AddBrowsingProduct(new Model.BrowsingHistoryInfo() { MemberId = userId, BrowseTime = item.BrowseTime, ProductId = item.ProductId });
                    }
                    catch
                    {
                        continue;
        ...
        public static List<ProductBrowsedHistoryModel> GetBrowsingProducts(int num, long userId = 0)
        {
            ...
            if (userId == 0)
            {
                var products = ServiceHelper.Create<IProductService>().GetProductByIds(productIdList.Select(a => a.ProductId))
                    .Where(d=>d.SaleStatus==Model.ProductInfo.ProductSaleStatus.OnSale 
                        && d.AuditStatus==Model.ProductInfo.ProductAuditStatus.Audited).ToArray()
            ...
            else
            {
                ...
                model = ServiceHelper.Create<IProductService>().GetBrowsingProducts(userId)
                    .Where(d => d.Himall_Products.SaleStatus == Model.ProductInfo.ProductSaleStatus.OnSale
                        && d.Himall_Products.AuditStatus == Model.ProductInfo.ProductAuditStatus.Audited)
                   ...
                   .Select(a => new ProductBrowsedHistoryModel()
               {
                   ...
                   ShopId = a.Himall_Products.ShopId
                   }).ToList();
            }
            return model;
        }
