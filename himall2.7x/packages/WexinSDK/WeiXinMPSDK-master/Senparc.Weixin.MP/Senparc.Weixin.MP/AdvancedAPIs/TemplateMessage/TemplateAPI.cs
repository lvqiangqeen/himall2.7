...
namespace Senparc.Weixin.MP.AdvancedAPIs.TemplateMessage
{
    ...
    public static class TemplateApi
    {
        ...
        /// <param name="timeOut">代理请求超时时间（毫秒）</param>
        /// <returns></returns>
        public static AddMessageTemplateResult AddMessageTemplate(string accessTokenOrAppId, string template_id_short,int timeOut = Config.TIME_OUT)
        {
            return ApiHandlerWapper.TryCommonApi(accessToken =>
            {
                const string urlFormat = "https://api.weixin.qq.com/cgi-bin/template/api_add_template?access_token={0}";
                var postData = new AddTempleteModel()
                {
                    template_id_short = template_id_short
                };
                return CommonJsonSend.Send<AddMessageTemplateResult>(accessToken, urlFormat, postData, timeOut: timeOut);
