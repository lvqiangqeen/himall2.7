...
namespace Senparc.Weixin.MP.AdvancedAPIs.Poi
{
    ...
    public static class PoiApi
    {
        ...
        public static PoiCategory GetCategory( string accessTokenOrAppId , int timeOut = Config.TIME_OUT )
        {
            return ApiHandlerWapper.TryCommonApi( accessToken =>
            {
                var url = string.Format("http://api.weixin.qq.com/cgi-bin/poi/getwxcategory?access_token={0}", accessToken);

                return CommonJsonSend.Send<PoiCategory>( null , url , null , CommonJsonSendType.GET , timeOut );
